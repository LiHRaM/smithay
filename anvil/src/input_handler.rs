...
impl AnvilState {
        ...
        let (x, y) = {
            #[cfg(feature = "udev")]
            {
                if self.session.is_some() {
                    ...
                    let screen_size = self.current_screen_size(x);
    ...
    fn clamp_coords(&self, pos: (f64, f64)) -> (f64, f64) {
        ...
        let max_y = self.current_screen_size(x).1;
    ...
    #[cfg(feature = "udev")]
    fn current_screen_size(&self, x: f64) -> (u32, u32) {
        let output_map = self.output_map
            .as_ref().unwrap();
        let outputs = output_map.borrow();

        let curr_output_idx = outputs
            .iter()
